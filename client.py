...
class ClientGUI:
    def __init__(self, master):
        ...
        # uruchomienie klienta w osobnym wątku
        threading.Thread(target=self.start_client, daemon=True).start()

    def start_client(self):
        # inicjalizacja i połączenie z hostem
        ...
        self.update_chat("Połączono z hostem.")

        # uruchomienie odbierania wiadomości w osobnym wątku
        threading.Thread(target=self.receive_messages, daemon=True).start()

    # odbieranie i deszyfrowanie wiadomości od hosta
    def receive_messages(self):
        while True:
            try:
                ...
                #self.update_chat(f"Host: {message}\nAES Nonce: {aes_nonce}\nSHA256: {sha256_hash}")
                self.update_chat(f"Host: {message}")
            except:
                break
        self.client_socket.close()

    # szyfrowanie i wysyłanie wiadomości do hosta
    def send_message(self):
        message = self.msg_entry.get()
        if message and self.client_socket and self.server_public_key:
            session_key = get_random_bytes(16)
            encrypted_message, aes_nonce, sha256_hash = encrypt_message(message, session_key, self.server_public_key)
